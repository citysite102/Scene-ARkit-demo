...
// MARK: Setup
    ...
    func setupAxisAlignedCamera(_ cameraNode: SCNNode) {
        ...
        let axisAlignConstraint = SCNTransformConstraint.positionConstraint(
            inWorldSpace: true, with: {(_ node: SCNNode, _ position: SCNVector3) -> SCNVector3 in
                ...
                return SCNVector3.init(constrainedPosition)
            })
    ...
    func ensureNoPenetrationOfIndex(_ index: Int) {
        ...
        for j in 0..<friendCount {
            ...
            let otherPos =  float3.init(friends[j].position)
            let v = otherPos - pos
    ...
    func registerGameController(_ gameController: GCController) {
        ...
        gamePadLeft!.valueChangedHandler = {(_ dpad: GCControllerDirectionPad,
            _ xValue: Float, _ yValue: Float) -> Void in
        ...
        buttonB?.valueChangedHandler = {(_ button: GCControllerButtonInput,
            _ value: Float,
            _ pressed: Bool) -> Void in
            
            print("Value Changed:\(value), Pressed Changed:\(pressed)")
            guard let strongController = weakController else {
                return
            }
